...
class MainIndexEntry(object):
    def __init__(self, mediafile):
        ...
        self.title_length = len(
            self._mediaFile.get_title().encode(STRING_ENCODING))
        self.title_char_length = 2
        # To be set later
        self.title_offset = 0

        self.shortdir_length = len(
            self._mediaFile.get_shortdir().encode("ascii"))
        self.shortdir_char_length = 1
        # To be set later
        self.shortdir_offset = 0

        self.shortfile_length = len(
            self._mediaFile.get_shortfile().encode("ascii"))
        self.shortfile_char_length = 1
        # To be set later
        self.shortfile_offset = 0

        self.longdir_length = len(
            self._mediaFile.get_longdir().encode(STRING_ENCODING))
        self.longdir_char_length = 2
        # To be set later
        self.longdir_offset = 0

        self.longfile_length = len(
            self._mediaFile.get_longfile().encode(STRING_ENCODING))
    ...
    def get_representation(self):
        return struct.pack(
            "<HHH HIII HHI HHI HHI HHI HHI I",
